...
新版安全探针上报 `--month-rotate` 后，面板会在每台机器自己的 `traffic_reset_timezone` 内判断重置日。当天会自动向 Telegram 发送一张简洁卡片，展示机器名、重置日、当前周期流量和剩余流量。面板每小时检查一次，并在数据库中按“机器 + 本地日期”去重，因此重启面板不会重复推送。
